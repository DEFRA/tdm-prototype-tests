...
describe.only('User is able to login', () => {
  ...
  it('Should be on the "Home" page', async () => {
    ...
    await expect(browser).toHaveTitle(`Notifications | Trade Data Matching ALVS V2 POC`)

    await movementsPage.movementsLink.click()
    await expect(browser).toHaveTitle(`Movements | Trade Data Matching ALVS V2 POC`)
  })
